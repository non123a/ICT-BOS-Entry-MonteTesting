...
def backtest_london_sweep_bos(df, london_window, ny_window, pivot, sl_mult, tp_mult, tf_filter, risk_per_trade, wfo_step=0):
    ...
    for day, day_df in df.groupby(df.index.date):
        ...
        if sweep_taken:
            ...
            # post = day_df.loc[day_df.index >= trade.entry_time]
            # post = df.loc[df.index >= trade.entry_time]
            # post = df.loc[
            #         (df.index >= trade.entry_time) &
            #         (df.index <= trade.entry_time + max_hold)
            #     ]
            # if post.empty:
            #     continue
            ...
            if exit_time is None:
                ...
                rr_val = pnl_dollars / risk_per_trade 
            # if exit_time is None:
            #     # force exit at end of max hold
            #     exit_time = post.index[-1]
            #     exit_price = float(post.iloc[-1]["Close"])
            #     exit_reason = "time_limit"

            #     pnl_dollars = (
            #         (exit_price - trade.entry_price) * trade_size
            #         if trade.side == "long"
            #         else (trade.entry_price - exit_price) * trade_size
            #     )
    ...
    if trades:
        ...
        # closes = int((tdf["exit_reason"].isin(["close", "time_limit"])).sum())
...
# MAIN EXECUTION
# ---------------------------
def forward_test_engine(df, london_window, ny_window,
                        pivot=11, sl_mult=1.0, tp_mult=3.0,
                        tf_filter="none",
                        start_balance=10000,
                        risk_pct=0.005,
                        spread=0.2,        # gold points
                        slippage=0.1):     # gold points
    ...
    trades_df, summary = backtest_london_sweep_bos(
        ...
        tp_mult=tp_mult,
        tf_filter=tf_filter,
        risk_per_trade=1  # we override sizing
    )
    ...
    for _, t in trades_df.iterrows():
        ...
        slip = np.random.uniform(0, slippage)

        if side == "long":
            entry_real = entry + spread + slip
        else:
            entry_real = entry - spread - slip

        risk_amount = balance * risk_pct
        risk_price = abs(entry_real - sl)

        if risk_price == 0:
            continue

        size = risk_amount / risk_price

        exit_price = t["exit_price"]

        if side == "long":
            pnl = (exit_price - entry_real) * size
        else:
            pnl = (entry_real - exit_price) * size
        ...
        trade_logs.append({
            ...
            "rr": pnl / risk_amount if risk_amount != 0 else 0
        })
    ...
    df_trades["drawdown"] = df_trades["balance"] - df_trades["peak"]

    df_trades["dd_pct"] = df_trades["drawdown"] / df_trades["peak"]


    worst_dd = df_trades["drawdown"].min()
    worst_dd_pct = df_trades["dd_pct"].min()
    avg_risk = df_trades["risk_amount"].mean()
    worst_dd_r = worst_dd / avg_risk if avg_risk != 0 else 0
    monthly_dd = df_trades.groupby("month")["drawdown"].min()
    # =========================
    # 🔻 LOSING STREAK
    # =========================
    ...
    # flatten columns
    hourly_stats.columns = ["pnl_sum", "pnl_avg", "trades", "avg_rr"]

    # winrate per hour
    hourly_stats["winrate"] = df_trades.groupby("hour")["pnl"].apply(lambda x: (x > 0).mean() * 100)

    # sort best to worst
    hourly_sorted = hourly_stats.sort_values(by="pnl_sum", ascending=False)

    print("\n===== HOUR PERFORMANCE =====")
    print(hourly_sorted)

    # best / worst hour
    best_hour = hourly_sorted.index[0]
    worst_hour = hourly_sorted.index[-1]
    ...
    # print(f"Avg Monthly PnL: {avg_monthly:.2f}")
    ...
    return df_trades, monthly_pnl, monthly_dd
...
def main():
    ...
    trades, monthly_pnl, monthly_dd = forward_test_engine(
        df,
        london_window,
        ny_window,
        pivot=13,
    ...
    if trades is not None:
        trades.to_csv(os.path.join(args.outdir, "forward_trades.csv"), index=False)
        monthly_pnl.to_csv(os.path.join(args.outdir, "monthly_pnl.csv"))
        monthly_dd.to_csv(os.path.join(args.outdir, "monthly_dd.csv"))
